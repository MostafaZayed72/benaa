    ...
    <div class="p-4 md:p-10 flex flex-col justify-center items-center my-auto h-screen"
      ...
      <div class="max-w-md mx-auto p-6 bg-violet-700 shadow-md rounded-lg my-auto">
        ...
        <form @submit.prevent="loginUser" class="space-y-4">
          ...
          <h1 class="text-center text-sm delayed underline cursor-pointer hover:text-blue-400">{{ $t('Forget password?') }}</h1>
          <h1 class="text-center text-sm">{{ $t("don't have an account?") }} <nuxtLink class="text-blue-300 hover:text-blue-400 delayed underline" to="/signup">{{ $t('signup now') }}</nuxtLink></h1>
        </form>
